...
from waypaper.translations import (
    Belarusian,
    Chinese,
    English,
    French,
...
class App(Gtk.Window):
    ...
    def init_ui(self) -> None:
        ...
        self.top_button_box = Gtk.Box(
            orientation=Gtk.Orientation.HORIZONTAL, spacing=60
        )
        ...
        self.top_row_alignment = Gtk.Alignment(
            xalign=0.5, yalign=0.0, xscale=0.5, yscale=0.5
        )
        ...
        self.top_container.pack_start(
            self.search_entry, expand=False, fill=False, padding=0
        )
        self.top_container.pack_start(
            self.clear_button, expand=False, fill=False, padding=0
        )
        self.top_container.pack_start(
            self.sort_combo, expand=False, fill=False, padding=0
        )
        self.top_container.pack_start(
            self.refresh_button, expand=False, fill=False, padding=0
        )
        self.top_container.pack_start(
            self.random_button, expand=False, fill=False, padding=0
        )
        self.top_container.pack_start(
            self.options_button, expand=False, fill=False, padding=0
        )
        self.top_container.pack_start(
            self.exit_button, expand=False, fill=False, padding=0
        )
        ...
        self.button_row_alignment = Gtk.Alignment(
            xalign=0.5, yalign=0.0, xscale=0.5, yscale=0.5
        )
    ...
    def create_options_menu(self) -> None:
        ...
        self.include_subfolders_checkbox = Gtk.CheckMenuItem(
            label=self.txt.msg_subfolders
        )
        self.include_subfolders_checkbox.set_active(self.cf.include_subfolders)
        self.include_subfolders_checkbox.connect(
            "toggled", self.on_include_subfolders_toggled
        )
        self.menu.append(self.include_subfolders_checkbox)

        # Create all subfolder toggle:
        self.include_all_subfolders_checkbox = Gtk.CheckMenuItem(
            label=self.txt.msg_all_subfolders
        )
        self.include_all_subfolders_checkbox.set_active(self.cf.include_all_subfolders)
        self.include_all_subfolders_checkbox.connect(
            "toggled", self.on_include_all_subfolders_toggled
        )
        ...
        self.show_path_in_tooltip_checkbox = Gtk.CheckMenuItem(
            label=self.txt.msg_show_path_in_tooltip
        )
        self.show_path_in_tooltip_checkbox.set_active(self.cf.show_path_in_tooltip)
        self.show_path_in_tooltip_checkbox.connect(
            "toggled", self.on_show_path_in_tooltip_toggled
        )
    ...
    def swww_options_display(self) -> None:
        ...
        if self.cf.swww_transition_type in SWWW_TRANSITION_TYPES:
            active_transition = SWWW_TRANSITION_TYPES.index(
                self.cf.swww_transition_type
            )
            self.swww_transitions_options.set_active(active_transition)
            self.swww_transitions_options.connect(
                "changed", self.on_transition_option_changed
            )
    ...
    def process_images(self) -> None:
        ...
        self.image_paths = get_image_paths(
            self.cf.backend,
            self.cf.image_folder_list,
            self.cf.include_subfolders,
            self.cf.include_all_subfolders,
            self.cf.show_hidden,
            self.cf.show_gifs_only,
        )
        ...
        if self.cf.sort_option in ["date", "daterev"]:
            self.image_paths.sort(
                ...
                reverse=(self.cf.sort_option == "daterev"),
            )
        ...
        for image_path in list(self.image_paths):
            ...
            image_name = get_image_name(
                image_path, self.cf.image_folder_list, self.cf.show_path_in_tooltip
            )
    ...
    def get_filtered_images(self) -> list:
        ...
        image_names = [
            name for name in self.image_names if search_query in name.lower()
        ]
        thumbnails = [
            self.thumbnails[i]
            for i in range(len(self.image_names))
            if search_query in self.image_names[i].lower()
        ]
        image_paths = [
            self.image_paths[i]
            for i in range(len(self.image_names))
    ...
    def load_image_grid(self) -> None:
        ...
        for index, [thumbnail, name, path] in enumerate(
            zip(thumbnails, image_names, image_paths)
        ):
    ...
    def set_selected_wallpaper(self, path: str) -> None:
        ...
        if self.cf.selected_wallpaper:
            threading.Thread(
                ...
                args=(self.cf.selected_wallpaper, self.cf, self.cf.selected_monitor),
            ).start()
    ...
    def on_mpv_sound_toggled(self, toggle) -> None:
        ...
        if self.cf.backend == "mpvpaper":
            subprocess.Popen(
                f"echo 'cycle mute' | socat - /tmp/mpv-socket-{self.cf.selected_monitor}",
                shell=True,
            )
        elif self.cf.backend == "gslapper":
    ...
    def on_mpv_pause_button_clicked(self, widget) -> None:
        ...
        if self.cf.backend == "mpvpaper":
            subprocess.Popen(
                f"echo 'cycle pause' | socat - /tmp/mpv-socket-{self.cf.selected_monitor}",
                shell=True,
            )
        elif self.cf.backend == "gslapper":
    ...
    def on_key_pressed(self, widget, event) -> bool:
        ...
        # Processing rest of the keys:
            ...
            self.selected_index = min(
                self.selected_index + self.cf.number_of_columns,
            ...
            self.selected_index = max(
                self.selected_index - self.cf.number_of_columns, 0
            )
            self.load_image_grid()
            self.scroll_to_selected_image()
            ...
            self.selected_index = min(
                self.selected_index + 1, len(self.image_paths) - 1
            )
            self.load_image_grid()
            self.scroll_to_selected_image()
    ...
    def on_window_clicked(self, widget, event) -> bool:
        ...
        if self.is_enering_text:
            ...
            if focused_widget is not None:
                ...
                widget_x, widget_y = (
                    focused_widget.translate_coordinates(self, 0, 0)
                    if focused_widget.translate_coordinates(self, 0, 0)
                    else (0, 0)
                )

                if (
                    x < widget_x
                    or x > widget_x + alloc.width
                    or y < widget_y
                    or y > widget_y + alloc.height
                ):
                    self.reset_input_fields()
                    return True
